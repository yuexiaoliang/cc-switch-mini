...
- **单二进制** (`cc-switch-mini`)，Web UI + 本地 SQLite 数据库。
...
- **同一份代码，两种构建**：上游 Tauri 桌面版继续可用,`cargo build -p cc-switch-mini-server` 即可生成无头服务端二进制。
...
| `--port` | `3000` | 监听端口 |
| `--data-dir` | `~/.local/share/cc-switch-mini` | 日志与临时文件(数据库位于上游 `~/.cc-switch/`) |
...
> **与上游一致** —— `--data-dir` 是 cc-switch-mini 独有的参数。SQLite 数据库与各 provider 配置文件 (`~/.hermes/`、`~/.claude/`、`~/.codex/`、`~/.gemini/`) 始终使用上游路径,可在两套程序之间无缝切换,无须迁移数据。
...
cc-switch 前端把缺失的命令视为 404 并回退到 “该功能不可用” 状态。cc-switch-mini 返回空值(`[]`、`null`、`false`)或日志消息,前端不会崩溃,只是隐藏对应按钮。这与项目 “不写业务逻辑” 的契约一致 —— dispatch 层是瘦壳。
...
包装脚本安装在 `~/.local/bin/cc-switch-mini-ctl`:

| 子命令 | 作用 |
| --- | --- |
| `cc-switch-mini-ctl start` | 启用并启动 user-systemd 服务 |
| `cc-switch-mini-ctl stop` | 停止服务 |
| `cc-switch-mini-ctl restart` | 优雅重启 (`update` 后自动调用) |
| `cc-switch-mini-ctl status` | 一行打印服务状态、监听端口、公网端点、FRP 隧道 |
| `cc-switch-mini-ctl logs` | `journalctl --user -u cc-switch-mini -f` |
| `cc-switch-mini-ctl update` | `git pull` + `pnpm install` + `cargo build --release` + 重装 + 重启 |
...
cc-switch-mini 完整沿用上游目录布局。你可以在桌面版与本服务之间无缝切换,无需导入导出:
...
| 日志 / 临时文件 | `~/.local/share/cc-switch-mini/` | 仅 cc-switch-mini |

## 架构

```
+-------------------+        +-----------------------------+
| 浏览器            |  HTTP  | cc-switch-mini              |
|                   |  ----> |                             |
|  - React SPA      |        |  Axum 路由                  |
...
| 任务 | 命令 |
| --- | --- |
| 启动开发服务 | `cargo run -p cc-switch-mini-server` |
| 启动到指定端口 | `cargo run -p cc-switch-mini-server -- --port 8080` |
| 前端类型检查 | `pnpm run typecheck` |
| 打包发布版 | `pnpm run build:renderer && cargo build --release -p cc-switch-mini-server` |
| 验证 dispatch 覆盖率 | `bash .ccsm/scripts/check-coverage.sh` |
| 运行单元测试 | `cargo test -p cc-switch-mini-server` |
